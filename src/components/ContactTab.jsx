      ...
      <div className="bg-[#f2f1f9] rounded-[20px] p-4 shadow-sm">
        ...
        <div className="space-y-3">
          ...
          {doctor?.receptionPhone && (
            <div className="bg-[#c3b8dc] rounded-2xl p-4 flex items-center gap-3 cursor-pointer hover:bg-[#b8a8d4] transition-colors"
                 onClick={() => {
                   const cleanPhone = doctor.receptionPhone.replace(/\s+/g, '').replace(/[^\d+]/g, '');
          ...
          {/* Reception WhatsApp */}
          {doctor?.receptionWhatsapp && (
            <div className="bg-[#c3b8dc] rounded-2xl p-4 flex items-center gap-3 cursor-pointer hover:bg-[#b8a8d4] transition-colors"
                 onClick={() => {
                   const cleanPhone = doctor.receptionWhatsapp.replace(/\s+/g, '').replace(/[^\d+]/g, '');
...
[Your Name]`;
                   ...
                   window.location.href = `mailto:${doctor.receptionEmail}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
                 }}>
              <div className="w-10 h-10 bg-black rounded-full flex items-center justify-center">
          ...
          {/* Show message if no reception contact info */}
          {(!doctor?.receptionPhone && !doctor?.receptionWhatsapp && !doctor?.receptionEmail) && (
            <div className="bg-gray-100 rounded-2xl p-4 text-center">
              <p className="text-sm text-gray-500">No reception contact information available</p>
            </div>
          )}
        </div>
      </div>
...
[Your Name]`;
      ...
      <div className="hidden md:flex md:gap-6 md:items-stretch">
        {/* Left Column - Location and Doctor wrapped together */}
...
[Your Name]`;
        ...
        <div className="flex flex-col space-y-4 flex-1">
          ...
          {/* Timing Section */}
          <div className="bg-[#f2f1f9] rounded-[20px] p-4 shadow-sm flex-1">
            {/* Edit Controls */}
            {isEditing && (
              <div className="mb-4 flex gap-3">
                <button
