export default function Blogs() {
  return (
    <div >
    <h2 className="text-black text-2xl font-bold mt-[24px] ml-[30px]">Blogs & Six buy and sell</h2>
    <div className="flex p-6 overflow-auto scrollbar-hide scroll-smooth gap-4" >
    <div className="flex-col flex bg-[#f2f1f9] rounded-4xl shadow min-w-[300px] "> 
       <div>
        <img src="/icons/news.webp" alt="news1" />
       </div>
            <div className="flex-col flex-1 p-4">
              <h2 className="text-xl text-black font-bold ">How the Toyota Hilux Balances 
              Work and Play for Drivers ?</h2>
              <h2 className="text-xl text-purple-600 mt-4 " >12 August 2025</h2>
              <p className="text-grey-500 mt-1">Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.</p>
            </div>
    </div>
    <div className="flex-col flex bg-[#f2f1f9] rounded-4xl shadow min-w-[300px]  "> 
       <div>
        <img src="/icons/news.webp" alt="news1" />
       </div>
            <div className="flex-col flex-1 p-4">
              <h2 className="text-xl text-black font-bold ">How the Toyota Hilux Balances 
              Work and Play for Drivers ?</h2>
              <h2 className="text-xl text-purple-600 mt-4 " >12 August 2025</h2>
              <p className="text-grey-500 mt-1">Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.</p>
            </div>
    </div>
    <div className="flex-col flex bg-[#f2f1f9] rounded-4xl shadow min-w-[300px]  "> 
       <div>
        <img src="/icons/news.webp" alt="news1" />
       </div>
            <div className="flex-col flex-1 p-4">
              <h2 className="text-xl text-black font-bold ">How the Toyota Hilux Balances 
              Work and Play for Drivers ?</h2>
              <h2 className="text-xl text-purple-600 mt-4 " >12 August 2025</h2>
              <p className="text-grey-500 mt-1">Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, quos.</p>
            </div>
    </div>
    </div>
  </div>
  );
}