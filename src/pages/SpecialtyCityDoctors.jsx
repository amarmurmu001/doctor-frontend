...
export default function SpecialtyCityDoctors() {
  ...
  const formattedCity = getLocationFromParam(city);
  ...
  const fetchDoctors = useCallback(async () => {
    try {
      ...
      const locationParams = getLocationApiParams(formattedCity);
      ...
      console.log('🏥 Fetching doctors from:', url, 'for city:', formattedCity, 'specialty:', formattedSpecialty);
      ...
      if (validDoctors.length === 0) {
        if (doctorsArray.length === 0) {
          setError(`No doctors found in ${formattedCity}.`);
        } else {
          setError(`No ${formattedSpecialty} doctors found in ${formattedCity}. Please try a different specialty or city.`);
        }
      }
  ...
  }, [formattedCity, formattedSpecialty]);

  useEffect(() => {
    // Fetch doctors when city or specialty changes
    if (formattedCity && formattedSpecialty) {
      fetchDoctors();
    }
  }, [fetchDoctors, formattedCity, formattedSpecialty]);
  ...
  function mapDoctorToCardProps(doctor, index = 0) {
    ...
    const doctorCity = doctor.address?.city || formattedCity;
      ...
      <div className="w-full h-screen bg-[#f4f4ff] flex flex-col">
        ...
        <div className="flex-1 px-3 sm:px-4 py-4 sm:py-6">
          <h2 className="text-base sm:text-lg font-bold mb-4 sm:mb-6 text-center">
            Finding {formattedSpecialty} Doctors in {formattedCity}...
          </h2>
      ...
      <div className="w-full h-screen bg-[#f4f4ff] flex flex-col">
        ...
        <div className="flex-1 px-3 sm:px-4 py-4 sm:py-6 flex items-center justify-center">
          <div className="text-center bg-red-50 rounded-lg p-8 border border-red-200 max-w-md">
            ...
            >
              Back to {formattedSpecialty} Doctors
            </button>
          </div>
        </div>
      </div>
    ...
    <div className="w-full h-screen bg-[#f4f4ff] flex flex-col">
      ...
      <div className="flex-1 px-3 sm:px-4 py-4 sm:py-6">
        ...
        <div className="max-w-7xl mx-auto">
          <h2 className="text-base sm:text-lg font-bold mb-4 sm:mb-6 text-center">
            {doctors.length > 0 ?
              `${doctors.length} ${formattedSpecialty} doctors found in ${formattedCity}` :
              `${formattedSpecialty} Doctors in ${formattedCity}`
            }
          </h2>
          ...
          {doctors.length === 0 && !loading && (
            <div className="text-center py-6 sm:py-8 bg-yellow-50 rounded-lg border border-yellow-200 mx-4 sm:mx-0">
              ...
              <p className="text-gray-600 text-sm px-4 sm:px-0">
                We couldn't find any {formattedSpecialty} doctors in {formattedCity} at the moment. Please try a different city or specialty.
              </p>
            </div>
          )}
