...
export default function DoctorSpecialtyProfile() {
  const { city, specialty, doctorName } = useParams();
  ...
  useEffect(() => {
    ...
    if (formattedDoctorName) {
      // For now, redirect to the main doctor profile page with the doctor name
      // In a real implementation, you would fetch the doctor by name/slug
      console.log('Doctor Specialty Profile:', {
        city: formattedCity,
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">
            {formattedDoctorName} - {formattedSpecialty}
          </h1>
          <p className="text-gray-600 mb-6">Location: {formattedCity}</p>
          <p className="text-gray-500">
            This doctor profile is being loaded. Please check back soon or contact support if you need immediate assistance.
          </p>
          <div className="mt-6 space-x-4">
            <button
              onClick={() => navigate(`/doctors/${city}/${specialty}`)}
